...
public class Enemy : MonoBehaviour
{
    ...
    {
        if (persuasionSliderPrefab == null) return;

        Transform canvasTransform = LevelManager.BubbleCanvas;
        if (canvasTransform == null)
        {
            Debug.LogError("BubbleCanvas not found in LevelManager.");
            return;
        }

        GameObject sliderGO = Instantiate(persuasionSliderPrefab, canvasTransform);
