...
public class PlayerItemManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        string dropKey = playerController.keyDrop.ToString().ToUpper();
        string pickUpKey = playerController.keyPickUp.ToString().ToUpper();

        //itemHUD.resetPanelText(HandType.Left,leftKey);
        //itemHUD.resetPanelText(HandType.Right,rightKey);
        itemHUD.resetPanelText(HandType.Both, pickUpKey);
    }
    ...
    {
        // New logic for separate Pick Up (E) and Drop (Q) keys
        if (Input.GetKeyDown(playerController.keyPickUp))
        {
            if (doubleHandItem == null)
            {
                ...
                if (target != null)
                {
                    TryPick(target, HandType.Both);
                }
            }
        }

        if (Input.GetKeyDown(playerController.keyDrop))
        {
            if (doubleHandItem != null)
            {
        ...
        // Use interaction key for interacting with items on the ground
        if (Input.GetKeyDown(playerController.keyInteraction))
        {
            PickableItem target = GetDetectedItem();
    ...
    void HandleItemInput()
    {
        // New simplified logic for single item (both hands)
        if (Input.GetKeyDown(playerController.keyLeftItem) || Input.GetKeyDown(playerController.keyRightItem))
        {
            if (doubleHandItem != null)
            {
                TryPlaceOrDrop(doubleHandItem);
            ...
            else
            {
                ...
                if (target != null)
                {
                    TryPick(target, HandType.Both); // Always try to pick as a two-handed item
                }
            }
        }
    ...
    */
    ...
    void EquipItem(PickableItem item, HandType hand)
    {
        ...
        // Simplified for Both Hands Only
        Transform parent = bothHandPoint;
        doubleHandItem = item;
        leftHandItem = null;
        rightHandItem = null;

        string dropKey = GetFriendlyKeyName(playerController.keyDrop);

        itemHUD.setHandType(UI_HandType.Both);

                //Key Use Logic
        string leftKey_use = GetFriendlyKeyName(playerController.keyLeftUse);
        itemHUD.setPanelText(HandType.Both, item.itemID, dropKey, leftKey_use , item.itemSize);
    ...
    public void ShowPickUpHint(PickableItem item)
    {
        ...
        if (item.itemSize == ItemSize.Heavy)
        {
            message = "Too Heavy";
            key = "";
        }
        else if (AnyHandOccupied())
        {
            message = "Hands Full";
            key = "";
        }
    ...
    {
        // New simplified logic for single item (both hands)
        // Using keyLeftUse as the single "Use" button for the demo
        if (Input.GetKeyDown(playerController.keyLeftUse))
        {
            if (doubleHandItem != null)
            {
                AttemptInteraction(doubleHandItem);
            }
        }
    }
    ...
    */
    ...
    public void ConsumeHeldItem()
    {
        if (doubleHandItem != null)
        {
            PickableItem itemToDestroy = doubleHandItem;
